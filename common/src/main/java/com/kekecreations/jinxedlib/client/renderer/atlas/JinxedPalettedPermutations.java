...
public record JinxedPalettedPermutations(List<SpriteSource> textures, Identifier paletteKey, Map<String, Identifier> permutations, String separator) implements SpriteSource {
    ...
    public void run(ResourceManager resourceManager, Output output) {
        ...
        HashMap<Identifier, DiscardableLoader> sourceOutputMap = new HashMap<>();
        Output sourceOutput = new Output() {
            @Override
            public void add(@NotNull Identifier location, @NotNull DiscardableLoader spriteSupplier) {
                DiscardableLoader supplier = sourceOutputMap.put(location, spriteSupplier);
                if (supplier != null) {
                    supplier.discard();
                }
            }
            ...
            public void removeAll(@NotNull Predicate<Identifier> predicate) {
                ...
                while (iterator.hasNext()) {
                    Map.Entry<Identifier, DiscardableLoader> entry = iterator.next();
    ...
    static record JinxedPalettedSpriteSupplier(LazyLoadedImage baseImage, Supplier<IntUnaryOperator> paletteSupplier, Identifier permutationLocation) implements DiscardableLoader {
